...
/**
 * Scroll-direction tracking for the nav morph.
 *
 * Directional with hysteresis: a meaningful downward scroll folds the pill, a
 * meaningful upward scroll opens it, and near the top of the page it is always
 * open. The threshold pair (8px of travel to switch, 24px of page to leave the
 * top) keeps slow or jittery scrolling from flickering the state, and the whole
 * thing runs one rAF-throttled read per scroll burst — no per-event work.
 */
function useNavCollapsed(): boolean {
 ...
 * an icon over a micro-label and lets the cells share the width evenly — so the
 * bar cannot overflow at 320px — while `md` collapses to a single row of
 * labelled chips. `md` starts at 768px, which is an iPad in portrait rather
 * than a mouse, so the collapsed chips stay 44px tall and the icon buttons stay
 * 44px square: still a touch target, just a denser one.
 */
const CELL =
  'nav-cell tap relative flex min-w-0 flex-1 flex-col items-center justify-center gap-[0.1875rem] rounded-[1.125rem] px-0.5 py-1.5 md:h-11 md:flex-none md:flex-row md:gap-0 md:rounded-full md:py-0';
const CELL_ICON = 'size-[1.375rem] shrink-0 md:size-[1.0625rem]';
const CELL_LABEL =
  'nav-label max-w-full truncate text-[0.5625rem] font-semibold tracking-[0.03em] md:text-[0.8125rem] md:font-medium md:tracking-normal';
const CELL_REST = 'text-mist-300 md:hover:bg-white/8 md:hover:text-white';

/**
 * The one piece of chrome every browsing page shares: a floating crystal-glass
 * pill, anchored to the bottom on a handset and to the top from `md` up.
 *
 * The pane is tuned to read as glass rather than frost — a low tint, a short
 * blur, and a brightness lift so it picks up light from bright artwork behind
 * it — and it morphs with scroll direction: scrolling down folds the four
 * destinations into the pill (leaving Logo · Search · Saved · Profile), reading
 * again near the top or scrolling up unfolds it. One fixed element, one state
 * class; all of the motion lives in CSS on the pane itself.
 */
        ...
        <ul className="flex w-full items-center md:w-auto md:gap-0.5">
          <li className="nav-brand shrink-0">
            <Link
              ...
              className="brand-tile tap flex size-10 items-center justify-center rounded-[0.8125rem] text-white md:size-10"
            >
          ...
          {DESTINATIONS.map(({ href, label, icon: Icon }) => {
            ...
            return (
              <li key={href} className="nav-dest flex min-w-0 flex-1 md:flex-none">
                <Link
                  href={href}
          ...
          <li aria-hidden className="nav-sep mx-1 hidden h-6 w-px shrink-0 bg-white/12 md:block" />
